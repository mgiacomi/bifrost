...
func defaultConfigPath(environment PathEnvironment) (string, error) {
	if runtime.GOOS == "darwin" {
		...
		return filepath.Join(environment.Home, "Library", "Application Support", "Bifrost Console", "config.yaml"), nil
	}
	...
	return filepath.Join(root, "bifrost-console", "config.yaml"), nil
}
...
func defaultWorkspaceParent(environment PathEnvironment) (string, error) {
	if runtime.GOOS == "darwin" {
		...
		return filepath.Join(environment.Home, "Library", "Caches", "Bifrost Console", "workspaces"), nil
	}
	...
	return filepath.Join(root, "bifrost-console", "workspaces"), nil
}
