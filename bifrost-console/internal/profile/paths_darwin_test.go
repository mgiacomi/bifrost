...
func TestDarwinDefaultsUseAbsoluteHomeRoot(t *testing.T) {
	...
	if configPath != filepath.FromSlash("/Users/developer/Library/Application Support/Bifrost Console/config.yaml") {
		t.Fatalf("config path=%q", configPath)
	}
	if workspaceParent != filepath.FromSlash("/Users/developer/Library/Caches/Bifrost Console/workspaces") {
