...
func TestProfileLockExcludesAnotherProcessUntilRelease(t *testing.T) {
	...
	command.Env = append(os.Environ(), "BIFROST_PROFILE_LOCK_HELPER=1", "BIFROST_PROFILE_LOCK_PATH="+path)
...
func TestProfileLockHelperProcess(t *testing.T) {
	if os.Getenv("BIFROST_PROFILE_LOCK_HELPER") != "1" {
		return
	}
	path := os.Getenv("BIFROST_PROFILE_LOCK_PATH")
