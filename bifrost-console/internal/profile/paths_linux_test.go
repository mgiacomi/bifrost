...
func TestLinuxDefaultsUseAbsoluteXDGRootsWithoutHome(t *testing.T) {
	...
	if configPath != filepath.FromSlash("/xdg/config/bifrost-console/config.yaml") {
		t.Fatalf("config path=%q", configPath)
	}
	if workspaceParent != filepath.FromSlash("/xdg/state/bifrost-console/workspaces") {
...
func TestLinuxDefaultsIgnoreRelativeXDGRoots(t *testing.T) {
	...
	if configPath != filepath.FromSlash("/home/developer/.config/bifrost-console/config.yaml") {
		t.Fatalf("config path=%q", configPath)
	}
	if workspaceParent != filepath.FromSlash("/home/developer/.local/state/bifrost-console/workspaces") {
		t.Fatalf("workspace parent=%q", workspaceParent)
	}
}
