...
func (realRunner) run(current phase, context pipelineContext) error {
	...
	case phaseViteBuild:
		return runCommand(context.paths.web, []string{"VITE_BIFROST_VERSION=" + context.productVersion}, "npm", "run", "build:web")
	...
	case phaseGoBuild:
		...
		ldflag := "-X=github.com/mgiacomi/bifrost/bifrost-console/internal/release.productVersion=" + context.productVersion
		return runCommand(context.paths.module, nil, "go", "build", "-trimpath", "-ldflags", ldflag, "-o", output, "./cmd/bifrost-console")
	default:
...
func executableName() string {
	if runtime.GOOS == "windows" {
		return "bifrost-console.exe"
	}
	return "bifrost-console"
}
