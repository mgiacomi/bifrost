...
func (failure *Failure) ConsoleError(scopeID string) *consolecore.Error {
	...
	case FailureAccess:
		return consolecore.NewError(consolecore.CodeTargetAccessBlocked, "The selected target denied access before Bifrost authentication.", scopeID, details, failure)
	case FailureIncompatible:
		details.ExpectedCompatibilityVersion = failure.Expected
		details.ObservedCompatibilityVersion = failure.Observed
		return consolecore.NewError(consolecore.CodeIncompatibleTarget, "The selected target uses a different Bifrost release.", scopeID, details, failure)
	case FailureInvalidArgument:
