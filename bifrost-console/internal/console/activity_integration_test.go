...
func TestActivitySSEStreamEndToEndWithTabHeader(t *testing.T) {
	...
	streamRequest.Header.Set("X-Bifrost-Console-Tab", bs.TabID)
	streamRequest.Header.Set("X-Bifrost-Console-CSRF", bs.CSRFToken)
	...
	if !strings.Contains(body, "bifrost.activity") {
		t.Fatalf("expected bifrost.activity namespace in stream output: %s", body)
	}
