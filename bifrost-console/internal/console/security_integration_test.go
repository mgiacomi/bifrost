...
func TestLiveConsoleArtifactRoutesDoNotLeakPathsOrCredentials(t *testing.T) {
	secret := "BIFROST_" + "TEST_APPLICATION_KEY_DO_NOT_LEAK_c3d9"
	root := t.TempDir()
	configPath := filepath.Join(root, "profile", "config.yaml")
	workPath := filepath.Join(root, "work")
	ownedProfile, err := profile.Open(configPath)
	...
	// small artifact body.
	artifactBody := []byte(`{"kind":"TRACE_STARTED","summary":"security-boundary"}
{"kind":"STEP_COMPLETED","summary":"done"}
`)
	traceMetadata := fmt.Sprintf(
		`{"targetScopeId":"scope-1","traceId":"trace-security","sessionId":"session-1","outcome":"SUCCEEDED","finalizedAt":"2026-07-25T12:00:00Z","sizeBytes":%d,"persistencePolicy":"PERSISTENT","applicationTraceExpiresAt":"2026-08-01T12:00:00Z"}`,
		len(artifactBody),
	)
	targetServer := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		response.Header().Set("X-Bifrost-Instance-Id", "11111111-1111-4111-8111-111111111111")
		...
		if strings.HasSuffix(request.URL.Path, "/traces/trace-security/artifact") {
			...
			response.Header().Set("Content-Disposition", `attachment; filename="bifrost-trace-trace-security.ndjson"`)
	...
	connectReq.Header.Set("X-Bifrost-Console-Tab", state.TabID)
	connectReq.Header.Set("X-Bifrost-Console-CSRF", state.CSRFToken)
	...
	acquireReq.Header.Set("X-Bifrost-Console-Tab", state.TabID)
	acquireReq.Header.Set("X-Bifrost-Console-CSRF", state.CSRFToken)
