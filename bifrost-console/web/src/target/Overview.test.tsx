...
test("overview presents established HTTP facts and safe incompatibility details", async () => {
  view.current = {
    ...
    error: new BrowserAPIError(
      "INCOMPATIBLE_TARGET",
      "The selected target uses a different Bifrost release.",
      409,
      "scope-1",
      {
  ...
  expect(screen.getByText(/Expected 0.1.0-SNAPSHOT; observed 0.1.0/)).toBeVisible();
...
test("overview explicitly confirms and submits target replacement", async () => {
  const replacementKey = "BIFROST_" + "REPLACEMENT_KEY_123456789012345";
