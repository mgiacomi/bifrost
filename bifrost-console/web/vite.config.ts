...
export function createViteConfig(input: ConfigInput): UserConfig {
  ...
  const version = input.environment.VITE_BIFROST_VERSION;
  if (production && (!version || version.includes("${") || version === "development")) {
    throw new Error("VITE_BIFROST_VERSION must be the exact resolved Bifrost product version");
  }
  ...
  if (!production) {
    const target = validateLoopbackOrigin(
      input.environment.BIFROST_CONSOLE_GO_ORIGIN ?? "http://127.0.0.1:7943",
    );
    config.server = {
      host: "127.0.0.1",
      port: 5173,
...
export function validateLoopbackOrigin(value: string): string {
  ...
  try {
    ...
    throw new Error("BIFROST_CONSOLE_GO_ORIGIN must be an absolute HTTP loopback origin");
  }
  const loopback = url.hostname === "127.0.0.1" || url.hostname === "[::1]";
  if (url.protocol !== "http:" || !loopback || url.username || url.password || url.pathname !== "/" || url.search || url.hash) {
    throw new Error("BIFROST_CONSOLE_GO_ORIGIN must be an HTTP origin using an explicit loopback IP");
  }
  return url.origin;
}
