...
The shared Go profile and workspace contract also applies across console crashes and restarts. MCP never scans or adopts an earlier Go process's `bifrost-console-work/transient` files. A new console process must acquire its profile lock and then its workspace lock, validate the workspace, and safely clean its transient subtree before reading the MCP credential, opening the listener, or serving browser or MCP requests. Any failure in that startup barrier terminates startup rather than producing a reduced-function state. After startup, loss of the workspace lock, safety guarantees, or required workspace I/O causes the Go host to stop admitting work, cancel in-flight browser and MCP operations, close its service, and exit with an error. MCP never continues as a status-only or non-analysis service after workspace failure. Evidence already downloaded explicitly by an MCP client is outside console workspace ownership and cannot be recalled or cleaned by Bifrost Console.
...
17. Browser and MCP trace operations use the same centralized Go artifact service. MCP cannot create a separate copy, handle, capacity pool, pinning policy, or eviction lifecycle for the same scope-bound application trace.
...
19. The managed workspace is a Go host invariant, not an MCP capability or availability state. Startup completes workspace locking, validation, and cleanup before serving; any later workspace-wide safety or I/O failure closes browser and MCP service and terminates the host.
...
- browser and MCP queries pin and refresh the same installed copy, and shared eviction or target-scope invalidation expires the same handle consistently for both adapters;
- truncated evidence;
