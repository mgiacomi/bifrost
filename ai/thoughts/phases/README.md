## Bifröst: The Agentic LLM-OS Framework (Master Spec)

### 1. The Core Paradigm: LLM-as-CPU
Bifröst treats the LLM as a reasoning CPU and the surrounding application as an operating system (kernel). This shifts the focus from "chatbots" to agentic computing.  
Bifröst is the bridge between the LLM and the Spring Boot application. It leverages Spring AI as the foundation for model interactions. It is part of a suite of products created by lokiscale.com
...
- **Internal Routing Layer:** Bifröst natively routes LLM instructions internally using an MCP-inspired capability mapping, rather than requiring an external MCP server.

### 2. Project Structure (Spring-Native Architecture)
Bifröst is a modern Java project designed as a Spring Boot Starter. By leaning fully into the Spring ecosystem, we dramatically reduce boilerplate, utilizing Spring AI for prompt execution and tool calling, Spring Security for RBAC, and Spring's `Resource` abstractions for File Systems.
...
Bifröst natively exposes four primary operations to the LLM:

- **callMethod:** Fast, deterministic Java execution via reflection on Spring beans flagged with `@SkillMethod`.
- **callSkill:** Logic-heavy delegation that spawns an isolated sub-agent in a background thread utilizing Spring AI's `ChatClient`.
...
Skills are defined in pure YAML: the YAML is the context/parameterization (presentation) and the Java code or Spring AI sub-agent is the execution (back-end).
...
- `mapping`: `target_id` linking the skill to a Spring bean or sub-agent logic
...
### 6. Execution Lifecycle & Model Tiering
...
- **Model tiering:** Skills request a `model_preference` (e.g., `heavy`, `light`). The framework maps these to configured Spring AI `ChatModel` beans.
